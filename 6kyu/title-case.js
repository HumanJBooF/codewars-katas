...
// or(b) considered to be an exception and put entirely into lower case unless it is the first word,
...
const titleCase = (title, minorWords) => {
    const lowerCaseWord = title.split(' ').map(word => word.toLowerCase());
    const minor = minorWords ? minorWords.split(' ').map(word => word.toLowerCase()) : [];
    const newTitle = [];
    ...
    lowerCaseWord.forEach((word, i) => {
        (!i || !minor.includes(lowerCaseWord[i]))
            ? newTitle.push(capitalize(lowerCaseWord[i]))
            : newTitle.push(lowerCaseWord[i])
    })
